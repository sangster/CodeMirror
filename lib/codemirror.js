...
(function(mod) {
  ...
  function directionChanged(cm) {
    ...
    cm.doc.iter(function(line) {
      if (!line.direction && line.order != null)
        line.order = null;
    });
    regChange(cm);
  }
  ...
  function drawSelectionRange(cm, range, output) {
    ...
    function drawForLine(line, fromArg, toArg) {
      ...
      iterateBidiSections(getOrder(lineObj, cm), fromArg || 0, toArg == null ? lineLen : toArg, function(from, to, dir) {
  ...
  function cursorCoords(cm, pos, context, lineObj, preparedMeasure) {
    ...
    var order = getOrder(lineObj, cm), ch = pos.ch;
  ...
  function coordsCharInner(cm, lineObj, lineNo, x, y) {
    ...
    var bidi = getOrder(lineObj, cm), dist = lineObj.text.length;
    var from = lineLeft(lineObj, cm), to = lineRight(lineObj, cm);
    var fromX = getX(from), fromOutside = wrongLine, toX = getX(to), toOutside = wrongLine;

    if (x > toX) return PosWithInfo(lineNo, to, toOutside, 1);
    // Do a binary search between these bounds.
    for (;;) {
      if (bidi ? to == from || to == moveVisually(lineObj, from, 1, false, getOrder(lineObj, cm)) : to - from <= 1) {
      ...
      if (bidi) {
        middle = from;
        for (var i = 0; i < step; ++i) middle = moveVisually(lineObj, middle, 1, false, getOrder(lineObj, cm));
      }
  ...
  function findPosH(doc, pos, dir, unit, visually) {
    ...
    function moveOnce(boundToLine) {
      if (visually)
        var next = moveVisually(lineObj, ch, dir, true, getOrder(lineObj, doc.cm));
      else
        var next = moveLogically(lineObj, ch, dir, true);
      if (next == null) {
        if (!boundToLine && findNextLine()) {
          if (visually) ch = (dir < 0 ? lineRight : lineLeft)(lineObj, doc.cm);
  ...
  CodeMirror.prototype = {
    ...
    setLineDirection: methodOp(function(handle, direction) {
      var cm = this;
      return changeLine(cm, handle, "direction", function(line, lineN) {
        if (line.direction == direction) return false;
        line.direction = direction;
        if (line.order != null) line.order = null;
        var otherDirection = direction == "rtl" ? "ltr" : "rtl";
        cm.removeLineClass(line, "wrap", "CodeMirror-" + otherDirection);
        cm.addLineClass(line, "wrap", "CodeMirror-" + direction);
        regLineChange(cm, lineN, "text");
        return true;
      });
    }),
  ...
  function buildLineContent(cm, lineView) {
    ...
    for (var i = 0; i <= (lineView.rest ? lineView.rest.length : 0); i++) {
      ...
      if (hasBadBidiRects(cm.display.measure) && (order = getOrder(line, cm)))
  ...
  function getOrder(line, cm) {
    var order = line.order;
    if (order == null)
      order = line.order = bidiOrdering(line.text, line.direction || (cm ? cm.options.direction : "ltr"));
    return order;
  }
  ...
  function lineLeft(line, cm) {
    var order = getOrder(line, cm);
    return order ? bidiLeft(order[0]) : 0;
  }
  function lineRight(line, cm) {
    var order = getOrder(line, cm);
  ...
  function lineStart(cm, lineN) {
    ...
    var order = getOrder(visual, cm);
    var ch = !order ? 0 : order[0].level % 2 ? lineRight(visual, cm) : lineLeft(visual, cm);
    return Pos(lineN, ch);
  }
  function lineEnd(cm, lineN) {
    ...
    var order = getOrder(line, cm);
    var ch = !order ? line.text.length : order[0].level % 2 ? lineLeft(line, cm) : lineRight(line, cm);
  ...
  function moveVisually(line, start, dir, byUnit, order) {
    if (!order) return moveLogically(line, start, dir, byUnit);
    var pos = getBidiPartAt(order, start), part = order[pos];
    var target = moveInLine(line, start, part.level % 2 ? -dir : dir, byUnit);

    for (;;) {
      if (target > part.from && target < part.to) return target;
      if (target == part.from || target == part.to) {
        if (getBidiPartAt(order, target) == pos) return target;
        part = order[pos += dir];
  ...
  var bidiOrdering = (function() {
    ...
    return function(str, direction) {
      if (!str || direction == "ltr" && !bidiRE.test(str)) return false;
      ...
      // Move outer whitespace to the outer direction.
      // FIXME this assumes direction=ltr. Also, it should probably be
